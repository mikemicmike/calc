...
export class HighscoresService {
  ...
  private _formatActivities(activitiesArray: string[]): PlayerActivities {
  ...
  private _formatSkills(skillsArray: string[]): PlayerSkills {
  ...
  private _separate(data: string): string[] {
    return data.split('\n');
  }

  private _parseToJSON(data: string): Player {
    const lines = this._separate(data);
    ...
    return {
      activities: this._formatActivities([
